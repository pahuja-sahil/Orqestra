...
async def repair_integration(ctx, integration_id: str):
    """
    ARQ worker task - repairs broken integrations.
    Uses single LLM call repair agent with proper backoff.
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            if result["success"]:
                ...
                await handle_failed_repair(integration, result["error"], user, attempt, db, ctx)
                
        except Exception as e:
            logger.error("repair_unexpected_error", name=integration.name, error=str(e))
            await handle_failed_repair(integration, str(e), user, attempt, db, ctx)
...
async def handle_failed_repair(integration: Integration, error: str, user: User, attempt: int, db: AsyncSession, ctx):
    ...
    if attempt >= MAX_REPAIR_ATTEMPTS:
        ...
        await ctx["redis"].enqueue_job("repair_integration", str(integration.id))
